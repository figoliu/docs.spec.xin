...
项目会自动开始初始化，初始化完成后，会在当前目录下创建一个名为vsproject1的文件夹，里面包含了项目的所有文件。

![alt text](image-6.png)

注意这里有一个Git初始化出错的问题，可以先忽略，后续再修正。也可以进入vsproject1文件夹，手动初始化Git仓库。

```
git config --global --add safe.directory D:/code/vsproject1
cd vsproject1
git init
```
